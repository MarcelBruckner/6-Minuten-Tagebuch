...
export default function BottomNav(props: { value: string, setValue: (value: string) => void }) {
    ...
    const handleChange = (event: React.SyntheticEvent | undefined, newValue: string) => {
        ...
        switch (newValue) {
            ...
            case formatDate(): navigate(`/${formatDate()}`); break;
    ...
    if (cookies.sechs_minuten_tagebuch_token) {
        return <Paper sx={{ position: 'fixed', bottom: 0, left: 0, right: 0 }} elevation={3}>
            <BottomNavigation value={props.value} onChange={handleChange}>
                ...
                <BottomNavigationAction label={isValueDate ? props.value : formatDate()} value={isValueDate ? props.value : formatDate()} icon={<CalendarToday />} />
