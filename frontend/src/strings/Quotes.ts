export const QUOTES = [
  "Wenn du 'ja' sagst, dann sei dir sicher, dass du nicht 'nein' zu dir selbst sagst. (Paulo Cuelho)",
  "Du bist nicht so weit gekommen, um nur so weit zu kommen.",
  "Das Leben ist zu kurz für irgendwann.",
  "Das kalte Wasser wird nicht wärmer, wenn du später springst.",
  "Tue heute etwas, worauf du morgen stolz sein kannst.",
  "Du kennst deine Grenzen erst, wenn du über sie hinausgewachsen bist.",
  "Die Tat unterscheidet das Ziel vom Traum.",
  "Man gibt nicht auf, wenn es schwierig wird: Man legt richtig los!",
  "Wer sich selbst alles zutraut, wird andere übertreffen.",
  "Ein Diamant ist ein Stück Kohle, das Ausdauer hatte.",
  "Niemand, der sein Bestes gegeben hat, hat es später bereut.",
  "Alles, was du willst, ist auf der anderen Seite der Angst.",
  "Schmerz ist vergänglich, Erfolg bleibt für immer.",
  "Freiheit beginnt dort, wo die eigene Angst endet.",
  "Chancen sind wie Sonnenaufgänge. Wer wartet, verpasst sie.",
  "Es sind deine Gedanken, die deine eigene Welt erschaffen.",
  "Mach es, bevor du bereust, es nicht getan zu haben.",
  "Tue genau das, was dich auf deinem Weg weiter bringt.",
  "Genieße den Augenblick, denn der Augenblick ist dein Leben.",
  "Bessere Aussichten schaffen bessere Einsichten.",
  "Wer zur Quelle will, muss gegen den Strom schwimmen.",
  "Der Weg aus Angst heraus geht immer durch Angst hindurch.",
  "Du bekommst nicht, was Du verdienst, sondern was Du verhandelst.",
  "Auch im Alphabet kommt Anstrengung vor Erfolg.",
  "Es ist möglich! Mehr brauchst du nicht wissen.",
  "Kleine Schritte sind besser als keine Schritte.",
  "Machen ist wie wollen - nur krasser.",
  "Risiko ist besser als Reue.",
  "Du kannst die Zukunft verändern mit dem, was du heute tust.",
  "Höre nicht auf, wenn es weh tut. Höre auf, wenn du fertig bist.",
  "Scheitern ist nicht das Gegenteil von Erfolg. Es ist ein Teil davon.",
  "Ehrgeiz ist die Fähigkeit, die Träume real werden lässt.",
  "Warte nicht auf Motivation. Sei du die Motivation für andere!",
  "Innen muss etwas brennen, damit außen etwas leuchten kann.",
  "Wem du die Schuld gibst, dem gibst du die Macht.",
  "Das Geheimnis des Erfolgs ist anzufangen.",
  "Sobald wir unseren Geist auf ein konkretes Ziel richten, kommt es uns entgegen.",
  "Nicht wie groß der erste Schritt ist zählt, sondern die richtige Richtung.",
  "Vergiss die Gründe, warum du scheitern könntest - konzentriere dich auf den einen Grund, warum du es schaffen wirst.",
  "Alles beginnt mit der Entscheidung, es zu versuchen.",
  "Für das Aufgeben ist es immer zu früh.",
  "Wer nicht kämpft, hat schon verloren.",
  "Mach es, bevor du bereust, es nicht getan zu haben.",
  "Erfolg hat 3 Buchstaben: T-U-N.",
  "Du schaffst das! Ende der Geschichte.",
  "Vorne ist immer da wo sich keiner auskennt.",
  "Wer Leistung will, muss Sinn bieten.",
  "Des Teufels liebstes Möbelstück: die lange Bank.",
  "Alleine sind wir Tropfen, zusammen aber ein Meer.",
  "Es gibt mehr Menschen, die aufgeben als jene, die scheitern.",
  "Hinfallen ist keine Schande, liegenbleiben schon.",
  "Wer aufhört, besser zu werden, hat aufgehört, gut zu sein. (Philip Rosenthal)",
  "Wenn du fliegen willst, musst du loslassen, was dich runterzieht.",
  "Egal, wie langsam du läufst: Du schlägst alle, die zuhause bleiben.",
  "Es geht nicht darum, der Beste zu sein. Es geht darum, besser zu sein als gestern.",
  "Zähle nicht jede Runde. Sorge dafür, dass jede Runde zählt!",
  "Wünsche es Dir nicht, arbeite dafür!",
  "Wenn mein Körper nicht mehr kann, zieht mein Wille ihn hoch.",
  "Trainiere, weil du deinen Körper liebst - nicht, weil du ihn hasst.",
  "Dein Körper kann alles schaffen. Es ist dein Geist, den du überzeugen musst.",
  "Was dich nicht herausfordert, wird dich nicht verändern. (Fred Devito)",
  "Was du heute denkst, wirst du morgen tun. (Lew Nikolajewitsch Tolstoi)",
  "Sei der Beste, der Du sein kannst!",
  "Was mich nicht umbringt, macht mich stärker. (Friedrich Nietzsche)",
  "Fange zu tun an, dann hast du auch die Kraft dazu. (Ralph Waldo Emerson)",
  "Du bist deine eigene Grenze - erhebe dich darüber. (Hafis)",
  "Manche Menschen wollen, dass es passiert. Manche wünschen sich, dass es passiert. Andere sorgen dafür, dass es passiert! (Michael Jordan)",
  "Um zu erreichen, was du noch nicht geschafft hast, musst du etwas tun, was du noch nicht getan hast.",
  "Aus kleinem Anfang entspringen alle Dinge. (Cicero)",
  "Wenn du aufgeben willst, denk daran, warum du angefangen hast.",
  "Wenn dir etwas wichtig ist, gibt es kein Aber.",
  "Willst du deine Träume verwirklichen, musst du zunächst daraus erwachen.",
  "Wo der Wille erwacht, da ist schon fast etwas erreicht. (Hugo von Hofmannsthal)",
  "Wer nicht aufpasst, verwandelt ein 'nicht jetzt' in ein 'niemals'.",
  "In einem Jahr wirst du dir wünschen, du hättest heute angefangen. (Karen Lamb)",
  "Je schwerer es dir fällt, einen Schritt zu machen, desto wichtiger ist es, dass du ihn tust.",
  "Wer in den Seilen hängt, kann auch erstmal schaukeln!",
  "Auch Wolkenkratzer haben mal als Keller angefangen.",
  "Kaffee wurde erfolgreich hinzugefügt, Körper kann gestartet werden.",
  "Das sind keine Augenringe. Das sind die Schatten großer Taten!",
  "Wenn uns das Leben in die Knie zwingt, tanzen wir Limbo!",
  "Wer am Morgen verknittert aufsteht, hat am Tag die besten Entfaltungsmöglichkeiten.",
  "Wenn der Weg holprig wird, steigt man nicht aus, sondern schnallt sich an.",
  "Dieser Moment im Job, wenn du denkst: 'Alpaka-Hirte in Peru wäre die bessere Berufswahl gewesen…'",
  "Wenn du keine Lust hast, immer wieder von vorne anzufangen, dann gib niemals auf.",
  "Nur wer etwas leistet, kann sich etwas leisten. (Michail Gorbatschow)",
  "Push yourself because no one else is going to do it for you.",
  "Great things never cam from comfort zones.",
  "Be so good they can't ignore you.",
  "In the end we only regret the chances we didn't take.",
  "If it doesn't open it's not your door.",
  "Love it, change it or leave it.",
  "If your dreams don't scare you, they are too small. (Richard Branson)",
  "Losers quit when they're tired. Winners quit when they've won.",
  "Don't wish for it, work for it.",
  "The first step is you have to say that you can. (Will Smith)",
  "Don't quit. Suffer now and live the rest of your life as a champion. (Muhammad Ali)",
  "For every reason it's impossible, there are hundreds of people who have faced the same circumstances and succeeded. (Jack Canfield)",
  "Strength does not come from winning. Your struggles develop your strengths. When you go through hardships and decide not to surrender, that is strength. (Arnold Schwarzenegger)",
  "Mach dein Ding. Du musst niemanden beeindrucken, außer dich selbst.",
  "Lass dir dein Leuchten nicht nehmen, nur weil es andere blendet.",
  "Sei die beste Version von dir selbst.",
  "Wir können den Wind nicht beeinflussen, aber die Segel anders setzen. (Aristoteles)",
  "Du musst das machen, wovon du glaubst: Das kann man nicht machen! (Eleanor Roosevelt)",
  "Wenn du an deiner Erfolgsgeschichte schreibst, gib den Stift niemand anderem!",
  "Stärke wächst nicht aus körperlicher Kraft, sondern aus unbeugsamen Willen. (Mahatma Gandhi)",
  "Aufzustehen und weiterzumachen, wenn es am schwersten fällt, zeigt wahre Größe.",
  "Ein kurzer Rückschlag steht dem langfristigen Erfolg nicht im Weg, er fördert ihn.",
  "Misserfolge sind Wegweiser auf dem Weg zum Erfolg. (C.S. Lewis)",
  "Egal, wie etwas ausgeht: Wenn du dabei alles gegeben hast, brauchst du dir niemals einen Vorwurf machen.",
  "Alle Träume können wahr werden, wenn wir den Mut haben, ihnen zu folgen. (Walt Disney)",
  "Geh deinen Weg und lass die Leute reden. (Dante Alighieri)",
  "Du verlierst nie. Entweder du gewinnst oder du lernst.",
  "Wer etwas will, findet Wege, wer etwas nicht will, findet Gründe.",
  "Wer aufhört, besser werden zu wollen, hört auf, gut zu sein.",
  "Der sicherste Weg zum Erfolg ist immer, es noch einmal zu versuchen.",
  "Neue Wege entstehen beim Gehen.",
  "Entweder wir finden einen Weg oder wir machen einen. (Hannibal)",
  "Wer immer tut, was er schon kann, bleibt immer das, was er schon war. (Henry Ford)",
  "Der einzige Mist, auf dem nichts wächst, ist der Pessimist. (Theodor Heuss)",
  "Niemand weiß, was er kann, bis er es probiert hat. (Publilius Syrus)",
  "Jede lange Reise beginnt mit dem ersten Schritt. (Laotse)",
  "Du musst nicht dein Leben ändern, du musst dein Ändern leben.",
  "Es gibt 2 Arten von Schwierigkeiten: Entweder du änderst sie oder sie ändern dich.",
  "Der Wert einer Idee liegt in ihrer Umsetzung. (Thomas Alva Edison)",
  "Wo auch immer du hingest, geh mit ganzem Herzen. (Konfuzius)",
  "Man kann niemanden überholen, wenn man in seine Fußstapfen tritt. (Francois Truffaut)",
  "Das Geheimnis des Könnens liegt im Wollen. (Giuseppe Mazzini)",
  "Lass die Angst vor dem Scheitern nicht größer sein als die Lust auf das Gelingen. (Robert Kiyosaki)",
  "Ein Schiff ist sicherer, wenn es im Hafen liegt. Aber dafür werden Schiffe nicht gebaut. (Paulo Coelho)",
  "Erfolg ist die Fähigkeit, von einem Misserfolg zum anderen zu gehen, ohne seine Begeisterung zu verlieren. (Winston Churchill)",
  "Ich messe meinen Erfolg nicht an Siegen, sondern daran, ob ich jedes Jahr besser werde. (Tiger Woods)",
  "Motivation ist der Zündschlüssel für Erfolg, Leidenschaft der beste Treibstoff. (Karl-Heinz Karius)",
  "Glaube, du kannst - und du bist auf halbem Weg. (Theodor Roosevelt)",
  "Jede schwierige Situation, die du jetzt meisterst, bleibt dir in der Zukunft erspart. (Dalai Lama)",
  "Einen Vorsprung im Leben hat, wer da anpackt, wo die anderen nur reden. (John F. Kennedy)",
  "Zeit ist begrenzt, also verschwende sie nicht, um das Leben eines anderen zu leben. (Steve Jobs)",
  "Es gibt nur zwei Tage im Leben, an denen du nichts ändern kannst: Der eine ist gestern und der andere ist morgen. (Dalai Lama)",
  "Fange nie an aufzuhören. Höre nie auf anzufangen. (Cicero)",
  "Jedes Jahr ist ein Buch mit 365 leeren Seiten.",
  "Wenn du es dir vorstellen kannst, kannst du es auch tun. (Walt Disney)",
  "Niemand, der sein Bestes gegeben hat, hat es später bereut.",
  "Probleme sind verkleidete Möglichkeiten.",
  "Glück ist kein Ziel, sondern eine Art zu leben.",
  "Wenn der Tag nicht dein Freund war, so war er doch dein Lehrer.",
  "Positive Dinge passieren nur positiven Menschen.",
  "Gib niemals auf, denn du weißt nie, wie nah du deinem Ziel wirklich bist.",
  "Auch aus Steinen, die einem in den Weg gelegt werden, kann man etwas Schönes bauen.",
  "Der Schmerz von heute ist deine Stärke von morgen.",
  "In einem Jahr wirst du dir wünschen, du hättest genau heute angefangen.",
  "Wer keine Ziele hat, kann nichts erreichen.",
  "Zeit ist begrenzt, also verschwende sie nicht - Carpe diem!",
  "Wenn der Plan nicht funktioniert, ändere den Plan - nicht das Ziel.",
  "Es gibt so viele bunte Farben: Warum also immer alles schwarz malen?",
  "Liebe ist nur ein Wort - bis jemand kommt und ihm eine Bedeutung gibt.",
  "Die schwersten Zeiten mit dir sind immer noch schöner, als die besten Zeiten, mit jemand anderem.",
  "Kleider machen Leute, aber das Herz macht den Menschen.",
  "Entfernung kann nur Körper trennen, aber niemals Herzen.",
  "Liebe ist wie ein Vollbad: Einlassen, warm halten & darin baden, bis man schrumpelig wird.",
  "Wünsche dir nicht, dass es einfacher wäre. Wünsche dir, dass du besser darin wirst. (Jim Rohn)",
  "Jede schwierige Situation, die du jetzt meisterst, bleibt dir in der Zukunft erspart. (Dalai Lama)",
  "Ich kann nicht sagen, ob es besser wird, wenn es anders wird. Aber so viel kann ich sagen: Es muss anders werden, wenn es besser werden soll. (Georg Christoph Lichtenberg)",
  "Wege entstehen dadurch, dass man sie geht. (Franz Kafka)",
  "Mit kleinen Hieben fällt man auch große Bäume. (Benjamin Franklin)",
  "Auch Wolkenkratzer haben mal ganz unten begonnen. (Hanno Nühm)",
  "Fange nie an aufzuhören. Höre nie auf anzufangen. (Cicero)",
  "Man kann niemanden überholen, wenn man in seine Fußstapfen tritt.",
  "Der eine wartet, dass die Zeit sich wandelt, der andere packt sie kräftig an und handelt. (Dante Alighieri)",
  "hätte, sollte, könnte, würde — MACHEN!",
  "Mach es, bevor du bereust, es nicht getan zu haben.",
  "Es ist möglich. Mehr brauchst du nicht zu wissen.",
  "Auf einfache Wege schickt man nur die Schwachen. (Hermann Hesse)",
  "Man muss das Unmögliche versuchen, um das Mögliche zu erreichen. (Hermann Hesse)",
  "Alle Träume können wahr werden, wenn wir den Mut haben, ihnen zu folgen. (Walt Disney)",
  "Du musst nicht spitze sein, um anzufangen. Aber du musst anfangen, um spitze zu werden. (Zig Zaglar)",
  "Wenn dir etwas wichtig ist, gibt es kein Aber.",
  "Die Kunst ist, einmal mehr aufzustehen, als man umgeworfen wird. (Winston Churchill)",
  "Die Tat unterscheidet das Ziel vom Traum.",
  "Wer kämpft, kann verlieren. Wer nicht kämpft, hat schon verloren. (Bertolt Brecht)",
  "Du musst genau das machen, wovon du glaubst: Das kann man nicht machen. (Eleanor Roosevelt)",
  "Stärke wächst nicht aus körperlicher Kraft — vielmehr aus unbeugsamem Willen. (Mahatma Gandhi)",
  "Du bist nicht so weit gekommen, um nur so weit zu kommen.",
  "Der Weg zum Erfolg beginnt mit einem einzigen Schritt.",
  "Träume groß, arbeite hart und mach es möglich.",
  "Die größten Erfolge kommen oft aus den größten Herausforderungen.",
  "Glaube an dich selbst und alles ist möglich.",
  "Erfolg ist die Summe von kleinen Anstrengungen, die jeden Tag wiederholt werden.",
  "Sei mutig genug, um deinen eigenen Weg zu gehen.",
  "Das Geheimnis des Erfolgs liegt darin, den eigenen Weg zu finden und unbeirrbar zu gehen.",
  "Das Leben belohnt diejenigen, die den Mut haben, ihren Träumen zu folgen.",
  "Jeder Tag ist eine neue Chance, deine Träume zu verwirklichen.",
  "Die einzige Grenze, die es gibt, ist die, die du dir selbst setzt.",
  "Erfolg hat drei Buchstaben: TUN!",
  "Ich messe meinen Erfolg nicht an Siegen, sondern daran, ob ich jedes Jahr besser werde. (Tiger Woods)",
  "Weniger zögern und mehr wagen, öfter innehalten, anstatt zu hasten, heute leben, anstatt zu verschieben, unsere Träume leben, anstatt unser Leben zu träumen. (Jochen Mariss)",
  "Nur wer selbst brennt, kann ein Feuer in anderen entfachen. (Augustinus)",
  "Du musst bereit sein, die Dinge zu tun, die andere niemals tun werden, um die Dinge zu haben, die andere niemals haben werden. (Les Brown)",
  "Habe keine Angst, das Gute aufzugeben, um das Großartige zu erreichen. (John D. Rockefeller)",
  "Mit Fleiß, mit Mut und festem Willen lässt jeder Wunsch sich endlich stillen. (Novalis)",
  "Der größte Ruhm im Leben liegt nicht darin, nie zu fallen, sondern jedes Mal wieder aufzustehen. (Nelson Mandela)",
  "Motivation ist der Zündschlüssel für Erfolg, Leidenschaft der beste Treibstoff. (Karl-Heinz Karius)",
  "Erfolg ist die Fähigkeit, von einem Misserfolg zum anderen zu gehen, ohne seine Begeisterung zu verlieren. (Winston Churchill)",
  "Eine positive Einstellung zu lösbaren Problemen ist bereits der halbe Erfolg. (Ernst Ferstl)",
  "Wenn jemand sagt: 'Das geht nicht', denk daran: Das sind seine Grenzen, nicht deine.",
  "Der Schmerz von Heute ist die Stärke von Morgen.",
  "Wir können den Wind nicht ändern, aber die Segel anders setzen. (Aristoteles)",
  "Wenn der Plan nicht funktioniert, ändere den Plan — nicht das Ziel.",
  "Es hängt von dir selbst ab, ob du das neue Jahr als Bremse oder als Motor benutzen willst. (Henry Ford)",
  "Wenn du an dich glaubst, kannst du Berge versetzen. (Norman Vincent Peale)",
  "Wenn du es dir vorstellen kannst, kannst du es auch tun. (Walt Disney)",
  "Zeit ist begrenzt, also verschwende sie nicht!",
  "Tue heute etwas, worauf du morgen stolz sein kannst!",
  "Was du heute kannst besorgen, das verschiebe nicht auf morgen.",
  "Niemand weiß, was er kann, bis er es probiert hat. (Publilius Syrus)",
  "Die Zukunft gehört denen, die an die Schönheit ihrer Träume glauben. (Eleanor Roosevelt)",
  "Wer kein Ziel hat, kann auch keines erreichen. (Laotse)",
  "Der größte Ruhm liegt nicht darin, niemals zu fallen, sondern jedes Mal wieder aufzustehen. (Ralph Waldo Emerson)",
  "Die Zukunft hängt davon ab, was du heute tust. (Mahatma Gandhi)",
  "Du wirst als Mensch nur wachsen, wenn du dich außerhalb deiner Komfortzone befindest. (Percy Cerutty)",
  "Wenn du Zeit hast, zu jammern und dich zu beklagen, hast du auch Zeit, etwas dagegen zu tun.",
  "Wenn es Dich nicht herausfordert, verändert es Dich nicht!",
  "Ein Schiff ist sicherer, wenn es im Hafen liegt. Aber dafür werden Schiffe nicht gebaut. (Paulo Coelho)",
  "Veränderung bedeutet Wachstum. Wenn du dich nicht veränderst, wirst du nie wissen, wie weit du gehen könntest.",
  "Manchmal ist Veränderung genau das, was wir brauchen, um uns selbst zu finden.",
  "Die Zukunft gehört denen, die sich auf die Veränderung vorbereiten, statt darauf zu warten, dass sie passiert. (Leon C. Megginson)",
  "Wir müssen bereit sein, uns von dem Leben zu lösen, das wir geplant haben, damit wir das Leben finden, das auf uns wartet. (Oscar Wilde)",
  "In einem wankenden Schiff fällt um, wer stillsteht und sich nicht bewegt. (Ludwig Börne)",
  "Wer nicht mutig genug ist, Risiken einzugehen, wird es im Leben zu nichts bringen! (Muhammad Ali)",
  "Nichts in der Geschichte des Lebens ist beständiger als der Wandel. (Charles Dickens)",
  "Alles, was du willst, ist auf der anderen Seite der Angst.",
  "Veränderung beginnt in dem Moment, in dem du beschließt, dass du sie willst.",
  "Sei nicht ängstlich vor Veränderung. Sie kann die Tür zu neuen Möglichkeiten öffnen, die du nie für möglich gehalten hast.",
  "Auch große Veränderungen beginnen mit einem kleinen Schritt!",
  "Nur wer sich ändert, bleibt sich treu. (Wolf Biermann)",
  "Veränderung ist unvermeidlich. Das Wachsen ist optional.",
  "Wenn alles gegen dich zu laufen scheint, erinnere dich daran, dass das Flugzeug gegen den Wind abhebt, nicht mit ihm. (Henry Ford)",
  "Der Glaube an unsere Kraft kann sie ins Unendliche verstärken. (Friedrich Schlegel)",
  "Auch im Alphabet kommt Anstrengung vor Erfolg.",
  "Gefühl von Grenze darf nicht heißen: hier bist Du zu Ende, sondern: hier hast Du noch zu wachsen! (Emil Gött)",
  "Das Geheimnis des Durchhaltens besteht darin, einen Schritt nach dem anderen zu machen. (Ban Ki-Moon)",
  "Nur wer sein Ziel kennt, findet den Weg. (Laozi)",
  "Erfolg kommt nicht von allein. Du musst dafür arbeiten, kämpfen und niemals aufgeben.",
  "In der Mitte von Schwierigkeiten liegen die Chancen. (Albert Einstein)",
  "Der Weg zum Erfolg ist gelegentlich gepflastert mit Niederlagen, Irrtümern, Hartnäckigkeit und harten Arbeit. (William Feather)",
  "Ehrgeiz ist die Fähigkeit, die Träume real werden lässt.",
  "Ob du glaubst, etwas erreichen zu können oder nicht — du liegst immer richtig. (Henry Ford)",
  "Wenn Du heute aufgibst, wirst Du nie wissen, ob Du es morgen geschafft hättest!",
  "Hab Geduld, alle Dinge sind schwierig, bevor sie einfach werden. (Saadi)",
  "Es ist nicht von Bedeutung, wie langsam du gehst, solange du nicht stehen bleibst. (Konfuzius)",
  "Wir sind verantwortlich für das, was wir tun, aber auch für das, was wir nicht tun!",
  "Erfolg ist nicht der Schlüssel zum Glück. Glück ist der Schlüssel zum Erfolg. Wenn du liebst, was du tust, wirst du erfolgreich sein. (Albert Schweitzer)",
  "Wer aufhört, besser zu werden, hat aufgehört, gut zu sein. (Philip Rosenthal)",
  "Probleme sind Gelegenheiten zu zeigen, was du kannst. (Duke Ellington)",
  "Ich bin nicht gescheitert - ich habe 10.000 Wege entdeckt, die nicht funktioniert haben. (Thomas Alva Edison)",
  "Misserfolge sind Wegweiser auf dem Weg zum Erfolg. (C.S. Lewis)",
  "Nicht wie tief du fällst zählt - sondern wie hoch du zurückfederst. (Zig Ziglar)",
  "Ich habe in meiner Karriere 9000 Würfe daneben geworfen. Ich habe fast 300 Spiele verloren. 26 Mal wurde mir der alles entscheidende Wurf anvertraut - und ich hab ihn verfehlt. Ich habe immer und immer wieder versagt in meinem Leben und daher war ich so erfolgreich! (Michael Jordan)",
  "Scheitern ist nicht das Gegenteil von Erfolg. Es ist ein Teil davon.",
  "Gewinner sind eigentlich Verlierer, die immer wieder aufstehen, wenn sie gefallen sind.",
  "Scheitern ist einfach nur eine Möglichkeit, es nochmals zu versuchen. Dieses Mal intelligenter. (Henry Ford)",
  "Du verlierst nie. Entweder Du gewinnst oder Du lernst!",
  "Hinfallen ist keine Schande, liegenbleiben schon!",
  "Nicht die Menschen, die immer gewinnen, sind die Stärksten, sondern die, die niemals aufgeben!",
  "Die größte Freude im Leben liegt oft in den einfachsten Dingen.",
  "Geh deinen Weg und lass die Leute reden. (Dante Alighieri)",
  "Mach dein Ding. Du musst niemanden beeindrucken, außer dich selbst.",
  "Jeden Tag gibt es etwas zu lächeln, wenn du nur genau hinsiehst.",
  "Wenn du an deiner Erfolgsgeschichte schreibst, gib den Stift niemand anderem!",
  "Genieße die kleinen Dinge im Leben, denn eines Tages wirst du zurückblicken und erkennen, dass sie die großen Dinge waren.",
  "Egal, wie etwas ausgeht: Wenn du dabei alles gegeben hast, brauchst du dir niemals einen Vorwurf machen.",
  "Believe you can and you're halfway there. (Theodore Roosevelt)",
  "Success is not final, failure is not fatal: It is the courage to continue that counts. (Winston Churchill)",
  "If your dreams don't scare you, they are too small. (Richard Branson)",
  "You miss 100% of the shots you don't take. (Wayne Gretzky)",
  "Don't watch the clock do what it does. Keep going.” (Sam Levenson)",
  "Don't wish for it, work for it.”",
  "The only limit to our realization of tomorrow will be our doubts of today.” (Franklin D. Roosevelt)",
  "The only way to achieve the impossible is to believe it is possible.” (Charles Kingsleigh)",
  "Don't quit. Suffer now and live the rest of your life as a champion.” (Muhammad Ali)",
  "You are never too old to set another goal or to dream a new dream.” (C.S. Lewis)",
  "Strength does not come from winning. Your struggles develop your strengths. When you go through hardships and decide not to surrender, that is strength.” (Arnold Schwarzenegger)",
  "Egal, wie langsam du läufst: Du schlägst alle, die zuhause bleiben.",
  "Wünsche es Dir nicht, arbeite dafür!",
  "Sei stärker als Deine stärkste Ausrede!",
  "Schmerz ist nur vorübergehend, aber der Stolz über das Erreichte währt ewig.",
  "Grenzen existieren nur in deinem Kopf. Wenn du glaubst, dass du etwas schaffen kannst, wirst du es auch tun.",
  "Der einzige Wettkampf, den du gewinnen musst, ist der gegen deine eigene Faulheit und Selbstzweifel.",
  "Wenn mein Körper nicht mehr kann, zieht mein Wille ihn hoch.",
  "Sei der Beste, der Du sein kannst!",
  "Wenn du etwas erreichen willst, was du noch nicht hast, musst du etwas tun, was du noch nicht getan hast.",
  "Der höchste Lohn für unsere Bemühungen ist nicht das, was wir dafür bekommen, sondern das, was wir dadurch werden. (John Ruskin)",
  "Der Geist ist stärker als der Körper. Wenn du glaubst, dass du es schaffen kannst, wirst du es tun.",
  "In deinen schwächsten Momenten entdeckst du, wie stark du wirklich bist.",
  "Manche Menschen wollen, dass es passiert. Manche wünschen sich, dass es passiert. Andere sorgen dafür, dass es passiert! (Michael Jordan)",
  "Was dich nicht herausfordert, wird dich auch nicht ändern. (Fred Devito)",
  "In einem Jahr wirst du dir wünschen, du hättest heute angefangen. (Karen Lamb)",
  "Zähle nicht jede Runde. Sorge dafür, dass jede Runde zählt!",
  "Dein Körper kann alles, es kommt nur darauf an, wie sehr dein Geist es will.",
  "Alle sagten: Es geht nicht. Da kam einer, der das nicht wusste und tat es einfach. (Goran Kikic)",
  "Dieser Moment im Job, an dem du dir denkst: Alpaka-Hirte in Peru wäre die bessere Berufswahl gewesen…",
  "Wer die Welt bewegen will, sollte erst sich selbst bewegen. (Sokrates)",
  "Wer versucht, sich immer alle Türen offen zu halten, wird sein Leben auf dem Flur verbringen.",
  "Wer heute den Kopf in den Sand steckt, knirscht morgen mit den Zähnen.",
  "Das Wort unmöglich gibt es nur im Wörterbuch von Narren. (Napoleon)",
  "Jeder Mensch mit einer neuen Idee ist ein Spinner. Bis die Idee Erfolg hat. (Mark Twain)",
  "Arbeite an dir selbst, nicht an deinem Selfie.",
  "Ein Diamant ist ein Stück Kohle, das Ausdauer hatte!",
  "Das kalte Wasser wird nicht wärmer, wenn Du später springst!",
];
